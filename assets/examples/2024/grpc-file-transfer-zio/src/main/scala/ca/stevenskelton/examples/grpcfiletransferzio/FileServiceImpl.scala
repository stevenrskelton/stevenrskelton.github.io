    ...
    val sink = ZSink.foldLeftZIO[Scope, StatusException | IOException, FileChunk, Option[SaveFileAccum]](None)((saveFileAccum, fileChunk) =>
      saveFileAccum match
        ...
        case Some(SaveFileAccum(_, _, _, offset)) if fileChunk.offset != offset =>
          ZIO.fail(IOException(s"Invalid chunk offset ${fileChunk.offset}, expected $offset"))
        ...
        case Some(SaveFileAccum(asyncFileChannel, file, totalSize, offset)) =>
          ...
          if saveFileAccum.offset > totalSize then
            ZIO.fail(IOException(s"Invalid chunk ${fileChunk.offset} exceeds total size $totalSize"))
          else
        ...
        case None =>
          ...
          if fileChunk.size > maxFileSize then
            ZIO.fail(IOException(s"File too large, attempted ${fileChunk.size} bytes"))
